@include('front.tender.head')
    <link rel="stylesheet" href="css/mycompetition.css">
</head>
<body >
<div class="wrap">
            ...
            <div class="right auction">
                <div v-for="(item, index) in model.list">
                    <div v-if="index == 0">
                        ...
                        <p class="tz-price">估价价：<span>@{{ item.tender_price }}</span></p>
                        {{--<p class="tz-price">投注：<span>2个<span>(小红花)</span></span></p>--}}
                        <p class="thisnum">估价时间：<span>@{{ item.created_at }}</span></p>
                    </div>
...
//                }
    ...
    function fetchGuessedTender(vueApp) {
        $.ajax('/tender/myguesses/' + vueApp.lastId, {
            method: 'get',
            dataType: 'json',
