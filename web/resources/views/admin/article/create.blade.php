...
@section('content')
    <div class="page-title">
        <h2><a href="{{route('article/index')}}"><i class="fa fa-reply"></i></a> 新增文章</h2>
    </div>
    ...
    <div class="col-md-12">
        ...
        <div class="panel panel-info">
            ...
            <div class="panel-body">
                <form class="form-horizontal" action="{{route('article/create')}}" role="form" method="post" enctype="multipart/form-data">
                    {{csrf_field()}}
                    <input type="hidden" name="articlable_id" value="{{request()->input('id')}}"/>
                    <input type="hidden" name="articlable_type" value="{{request()->input('type')}}"/>
                    {!! Form::bsText(['label' => '标题', 'name' => 'title', 'value' => old('name'), 'placeholder' => "请输入文章标题名称", 'ext'=>'required']) !!}
                    {!! Form::bsFile(['name' => 'picture', 'id' => 'filename2','value'=>old('picture'), 'title' => "上传图片",'url'=>[]]) !!}
                    <div class="form-group">
                        <label class="col-md-2 control-label">是否展示</label>
                        <div class="col-md-4">
                            <label class="check"><input type="radio" value="1" class="iradio" name="is_show" checked/> 是</label>
                            <label class="check"><input type="radio" value="0" class="iradio" name="is_show"/> 否</label>
                        </div>
                    </div>
                    <div class="form-group">
